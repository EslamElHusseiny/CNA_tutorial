...
docker run --name $MINIO_DOCKER_NAME -d --net shared_nw --ip 192.168.58.6 -e MINIO_ACCESS_KEY=$MINIO_ACCESS_KEY -e MINIO_SECRET_KEY=$MINIO_SECRET_KEY minio/minio server /data
docker run --rm --link $MINIO_DOCKER_NAME:minio --net shared_nw --ip 192.168.58.11 -e MINIO_BUCKET=$MINIO_BUCKET --entrypoint sh minio/mc -c "\
